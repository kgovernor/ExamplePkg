...
# ╔═╡ 527259cd-aadc-4ade-9f02-46510e0b2eaa
using IntervalArithmetic, IntervalRootFinding, TaylorModels, Plots, PlutoUI, LinearAlgebra

# ╔═╡ 849cc180-3dd8-11ee-32ab-f5ecd983bd3d
md"""
# Interval Arithmetic and Taylor Series Expansion
"""

# ╔═╡ 2e4a04aa-7975-49e9-95bf-5f87f333b79b
md"""
## Interval Arithmetic
"""

# ╔═╡ 0eaa2bae-5bb2-431f-aa70-b79f7635b345
begin
	ex = 1..10
	md"""
	$(mid(ex)),
...
# ╔═╡ 7f4ed411-5d9c-4836-a203-c5cdf7cd95a7
begin
	A = [1..2 3..4 ; 5..9 10..19]
	b = [1..1, 2..2]
	det(A)
...
# ╔═╡ a244a950-39e5-41f3-a0c9-c1d9ff5dacc5
md"""
## Interval Root Finding
"""

# ╔═╡ 38b11280-1629-4267-86e5-9ee2a1fc8971
plot(range(-10, stop=10, length=1000), x->sin(x), legend=false)

# ╔═╡ 90b97462-15bf-4a21-ac63-b5debf009ad4
roots(x->sin(x), -10..10)

# ╔═╡ 3eeee50e-1ae4-4c19-88a9-e32f14b4aa2d
md"""
## Taylor Series Expansion
"""

# ╔═╡ 4fbe703c-ee62-4fd8-bb7c-35dfce3642ae
@bind order Slider(1:1:10, default = 2)

# ╔═╡ 637d89b9-808b-418d-9ca0-efb8c8ee53c2
md"""
order = $(order)
"""

# ╔═╡ 4ea94a44-f3d9-48b5-9ace-21735803688e
begin
	f(x) = x*(x-1.1)*(x+2)*(x+2.2)*(x+2.5)*(x+3)*sin(1.7*x+0.5)
	a =  -0.5 .. 1.0 # Domain
	x0 = mid(a)     # Expansion point
	tm = TaylorModel1(order, interval(x0), a) # Independent variable for Taylor models
	# Taylor models corresponding to f(x) of order n
	ftm = f(tm)
	
	# Now the plot
	plot(range(inf(a), stop=sup(a), length=1000), x->f(x), label="f(x)", lw=2, xaxis="x", yaxis="f(x)")
	plot!(ftm, label="order $(order)")
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "c74f08ef99f559f5d5bebf6c40e96cfb48717e85"
...
deps = ["BenchmarkTools", "Dates", "DiffEqBase", "Elliptic", "Espresso", "InteractiveUtils", "LinearAlgebra", "Logging", "Markdown", "OrdinaryDiffEq", "PkgBenchmark", "RecursiveArrayTools", "Reexport", "Requires", "StaticArrays", "TaylorSeries"]
git-tree-sha1 = "0306c8bcd3adea14db595363df9cca717893d618"
...
git-tree-sha1 = "b16bf34546f0070a5c79c4a26b1535b72c6dcc34"
